...
const mockFavorites = [
  {
    ...
    answer: "根据《医疗器械注册与备案管理办法》，医美针剂作为第三类医疗器械，注册申报需要准备以下材料：1. 注册申请表；2. 证明性文件；3. 医疗器械安全有效基本要求清单...",
  ...
  },
  {
    ...
    answer: "透明质酸类产品的分类主要依据其预期用途和作用机理。用于注射填充的透明质酸钠产品，按照第三类医疗器械管理...",
    source: "《医疗器械分类目录》",
    savedTime: "昨天 16:42",
  },
  {
    ...
    answer: "有效期验证应按照《医疗器械稳定性研究技术审查指导原则》进行，包括实时稳定性研究和加速稳定性研究...",
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="mb-2 text-foreground">还没有收藏</p>
...
function NoResultsState({ query }: { query: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="mb-2 text-foreground">未找到相关收藏</p>
...
function FavoriteItem({
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(`问：${item.question}\n\n答：${item.answer}\n\n来源：${item.source}`)
    setCopied(true)
  ...
  return (
    <div className="glass overflow-hidden rounded-xl">
      ...
      {/* 回答摘要 */}
      <div className="p-4">
        <p className="mb-3 text-sm leading-relaxed text-secondary-foreground line-clamp-3">
          {item.answer}
        </p>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className={cn(
                "flex h-8 w-8 items-center justify-center rounded-full transition-all hover:bg-secondary/50 active:scale-95",
      ...
      <Link
        ...
        className="flex items-center justify-center border-t border-border/50 px-4 py-3 text-sm text-primary hover:bg-secondary/30"
      >
        查看完整回答
      </Link>
    </div>
